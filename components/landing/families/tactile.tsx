...
export function TactileLanding({ theme }: { theme: ThemeDefinition }) {
  ...
  const isAmbient = theme.recipe.heroVariant === "ambient-cloud";
  const isBotanical = theme.recipe.heroVariant === "earthy-asymmetry";
          ...
          <div className="grid gap-8 lg:grid-cols-[1.02fr_0.98fr] lg:items-center">
            <div>
              ...
              <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--theme-muted)] sm:text-xl">
                {isAmbient
                  ? `${productName} is pulled into atmosphere first here, so the interface feels suspended in soft color rather than built from obvious objects.`
                  : isBotanical
                    ? `${productName} becomes grounded and restorative, using natural asymmetry and earthy material cues instead of synthetic polish.`
            ...
            <div className="space-y-4">
              ...
              <Surface className="p-6 shadow-[var(--theme-shadow)] sm:p-7">
                <p className="text-xs uppercase tracking-[0.24em] text-[var(--theme-muted)]">Material feel</p>
              ...
              <Surface className="translate-y-4 p-6 shadow-[var(--theme-shadow)] sm:mt-10">
                <p className="text-xs uppercase tracking-[0.24em] text-[var(--theme-muted)]">Supporting treatments</p>
              ...
              <Surface className="p-6 shadow-[var(--theme-shadow)] sm:col-span-2">
                <p className="text-xs uppercase tracking-[0.24em] text-[var(--theme-muted)]">{proofLabel}</p>
                <div className="mt-5 grid gap-3 sm:grid-cols-3">
                  {theme.previewBullets.map((bullet, index) => (
                    ...
                    >
                      <p className="text-xs uppercase tracking-[0.2em] text-[var(--theme-muted)]">0{index + 1}</p>
      ...
      <section className="py-10 sm:py-14">
        <Container className="grid gap-8 lg:grid-cols-[0.82fr_1.18fr]">
          <SectionHeading
            ...
            title="Tactility here comes from surface treatment, rhythm, and layout behavior together."
            description="These directions stay soft for different reasons: one is object-heavy and molded, another is ambient and atmospheric, another is grounded in natural asymmetry."
          />
          <div className="grid gap-4 sm:grid-cols-2">
            {sharedContent.features.map((feature, index) => (
              ...
              >
                <p className="text-xs uppercase tracking-[0.22em] text-[var(--theme-muted)]">soft block 0{index + 1}</p>
        ...
        <Container>
          <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <Surface className="p-8 shadow-[var(--theme-shadow)]">
              <p className="text-xs uppercase tracking-[0.24em] text-[var(--theme-muted)]">Structural read</p>
              ...
              <p className="mt-6 text-sm leading-7 text-[var(--theme-muted)]">Audience fit: {theme.audience}</p>
            </Surface>
          </div>
        </Container>
      </section>
